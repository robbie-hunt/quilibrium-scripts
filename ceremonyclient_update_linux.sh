...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    RELEASE_OS="linux"
    echo "Release type determined as linux-gnu"
    if [[ $(uname -m) == "aarch64"* ]]; then
        RELEASE_ARCH="arm64"
        echo "Release architecture determined as arm64"
    else
        RELEASE_ARCH="amd64"
        echo "Release architecture determined as amd64"
    fi
else
    RELEASE_OS="darwin"
    echo "Release architecture determined as darwin"
    RELEASE_ARCH="arm64"
    echo "Release architecture determined as arm64"
fi
