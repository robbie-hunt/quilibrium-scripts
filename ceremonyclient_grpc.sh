...
CHECK_MODIFY_LISTEN_MULTIADDR_func() {
    ...
    if grep -q "^[[:space:]]*listenMultiaddr:[[:space:]]*/ip4/0\.0\.0\.0/udp/8336/quic" .config/config.yml; then
        ...
        if grep -q "^[[:space:]]*listenMultiaddr:[[:space:]]*/ip4/0\.0\.0\.0/tcp/8336" .config/config.yml; then
            ...
            echo "Error: Neither old nor new listenMultiaddr found. This could cause issues. Please manually check your config.yml file."
            return 1
        fi
    fi
}
...
SETUP_LOCAL_GRPC_func() {
    ...
    sudo sed -i -E 's|^listenGrpcMultiaddr: \"\"|listenGrpcMultiaddr: \"/ip4/127.0.0.1/tcp/8337\"|' .config/config.yml
    sudo sed -i -E 's|^listenRESTMultiaddr: \"\"|listenRESTMultiaddr: \"/ip4/127.0.0.1/tcp/8338\"|' .config/config.yml
...
SETUP_STATS_COLLECTION_func() {
    ...
    if ! LINE_EXISTS_func "statsMultiaddr: \"/dns/stats.quilibrium.com/tcp/443\"" .config/config.yml; then
        ADD_LINE_AFTER_PATTERN_func "engine" "statsMultiaddr: \"/dns/stats.quilibrium.com/tcp/443\"" .config/config.yml
